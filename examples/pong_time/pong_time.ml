...
struct
  ...
  let compare state1 state2 =
    ...
    | Ingame s1, Ingame s2->
        if s1.field.score > s2.field.score then 1
        else if s1.field.score < s2.field.score then -1
        else 0
    | _ -> 0

end

module StockablePong = Timebis.TIMELINE(Pong)

let _ =
  let fen = Mjwindow.create_window 400. 400. "Pong" 60 Color.black in
  StockablePong.play fen			    (*fenetre*)	
		     StockablePong.DrawThenEvent    (*ordre de déroulement*)
		     2				    (*maxbranche*)
